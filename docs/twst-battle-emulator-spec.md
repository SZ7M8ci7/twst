...
- ATK 系は `card.atk` に対する加算値として合算する。敵・特別課題からのATK DOWNも元ATKに対する減算とし、UP/DOWN合計後の有効ATKを0以上に制限する。DOWN率自体は100%で制限しない。
...
- `calculateDamage` の期待値用クリティカル倍率は、複数あっても最大倍率のみ採用する。試験シミュレーターのヒット単位の発動率計算とは別仕様。
...
- 通常攻撃の実戦 HP 推移再現: 各ヒットの非負ダメージを小数点以下切り捨てにする。回復・やけどの丸めは別規則。
...
試験シミュレーターでは、対象キャラに有効な複数のクリティカル発動率を加算し、合計を `100%` で上限にする。その合計率でヒットごとにクリティカル発動判定を行い、発動したヒットだけ `1.25` 倍する。デュオは 3 回攻撃として扱うため、クリティカル判定も 3 回行う。
...
- `凍結無効` は既に凍結中でも発動でき、効果中は凍結を無効化する。
- 凍結無効が有効な対象には無効。
- バフの凍結判定は付与対象ごとに行う。`味方選択`、`味方全体` で一部の対象だけが凍結無効の場合、凍結中の対象には付与せず、凍結無効の対象にだけ付与する。
- ATK 試験のバフ加点は、対象別判定後に1対象以上へ加点対象バフを付与できた場合、効果1つにつき1回だけ数える。
- 既存の単発計算画面では凍結を戦闘解決しない。試験シミュレーターでは凍結付与時に成功判定を行い、有効中は凍結後に発動する味方バフを抑止する。

### 状態異常無効

- `暗闇無効`、`呪い無効`、`凍結無効`、`やけど無効` は、対応する状態異常より先に付与されても後に付与されても、効果中はその状態異常を無効化する。
- 無効効果は対応する状態異常そのものを解除しない。状態異常の残りターンは無効効果中も進行する。
- 無効効果が終了した時点で対応する状態異常の残りターンがある場合、その状態異常は再び有効になる。
- 無効効果中に対応する状態異常を新たに付与された場合も、成功判定と継続ターンを内部状態として保持する。無効効果終了後に継続ターンが残っていれば有効になる。
...
- ダメージ乱数は `0.95..1.05` の一様分布、通常攻撃の丸めは各ヒットで小数点以下切り捨てで実装する。
...
- プロジェクト互換なら乱数なし固定ダメージモード。実ゲーム再現なら `0.95..1.05` 一様乱数と通常攻撃のヒットごとの小数点以下切り捨て。
...
8. ダメージ乱数は `0.95..1.05` 一様分布、通常攻撃の丸めは各ヒットで小数点以下切り捨て。
