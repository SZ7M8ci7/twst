...
function restoreInitialExamDeck() {
  ...
  if (resourceTransfer) {
    void prepareResourceCandidate(resourceTransfer.input, resourceTransfer.candidate).then(async () => {
      ...
      const best = resourceTransfer.best;
      const replay = best ? runResourceTrial(best.seed, resourceTransfer.candidate.policy, true) : null;
      const plan = replay && replay.score === best?.score ? replay.plan ?? [] : [];
      turnPlans.value = Array.from({ length: maxTurnCount.value }, (_, index) => ({
        turn: index + 1,
        useElementCompatibilityPriority: false,
        combos: [createTurnCombo(plan[index]?.[0] ?? '', plan[index]?.[1] ?? '')],
      }));
      simulationMode.value = 'normal';
...
function resetDefaultCombos() {
  ...
  if (ids.length < 2) {
    turnPlans.value.forEach((turn) => {
      if (turn.combos.every((combo) => combo.autoGenerated)) {
